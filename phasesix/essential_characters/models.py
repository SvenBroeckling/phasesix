...
class EssentialCharacter(ModelWithImage):
    ...
    image = models.ImageField(
        upload_to=image_upload_path, max_length=256, blank=True, null=True
    )
    ...
    ancestry = models.ForeignKey(
        EssentialAncestry, null=True, blank=True, on_delete=models.SET_NULL
    )
    path = models.ForeignKey(
        EssentialPath, null=True, blank=True, on_delete=models.SET_NULL
    )
    bond = models.ForeignKey(
        EssentialBond, null=True, blank=True, on_delete=models.SET_NULL
    )
    ...
    spells = models.ManyToManyField(
        "EssentialSpellProfile", through="EssentialCharacterSpell", blank=True
    )
    items = models.ManyToManyField(
        "armory.Item", through="EssentialCharacterItem", blank=True
    )
    weapons = models.ManyToManyField(
        "EssentialWeaponProfile", through="EssentialCharacterWeapon", blank=True
    )
    armor = models.ManyToManyField(
        "EssentialArmorProfile", through="EssentialCharacterArmor", blank=True
    )
    ...
    def validate_assignments(self):
        ...
        if (
            ...
            or self.essentialcharacterspell_set.count() > slots["spells"]
        ):
            raise ValidationError(
    ...
    def invocation_value(self):
        rite = (
            self.essentialcharacterskill_set.filter(skill__name__icontains="ritus")
        ...
        with transaction.atomic():
            ...
            for assignment in self.essentialcharacterskill_set.all():
                EssentialCharacterSkill.objects.create(
                    character=clone, skill=assignment.skill, rank=assignment.rank
                )
            ...
            for assignment in self.essentialcharacterweapon_set.all():
                EssentialCharacterWeapon.objects.create(
                    character=clone, profile=assignment.profile, slot=assignment.slot
                )
            for assignment in self.essentialcharacterarmor_set.all():
                EssentialCharacterArmor.objects.create(
                    character=clone,
                    profile=assignment.profile,
                    equipped=assignment.equipped,
                )
            for assignment in self.essentialcharacteritem_set.all():
                EssentialCharacterItem.objects.create(
                    character=clone, item=assignment.item, quantity=assignment.quantity
                )
            for assignment in self.essentialcharacterspell_set.all():
                EssentialCharacterSpell.objects.create(
                    character=clone, profile=assignment.profile
                )
            return clone
...
class EssentialSpellProfile(models.Model):
    ...
    aspect = models.ForeignKey(
        EssentialMagicAspectProfile, null=True, blank=True, on_delete=models.SET_NULL
    )
