...
def forward(apps, schema_editor):
    ...
    for wiki_page in WikiPage.objects.all():
        if wiki_page.text_de and "{{" in wiki_page.text_de:
            wiki_page.text_de = re.sub(
                r"w-25,float-end", "image-end", wiki_page.text_de
            )
            wiki_page.text_de = re.sub(
                r"w-25,float-start", "image-start", wiki_page.text_de
            )
            wiki_page.save()
        if wiki_page.text_en and "{{" in wiki_page.text_en:
            wiki_page.text_en = re.sub(
                r"w-25,float-end", "image-end", wiki_page.text_en
            )
            wiki_page.text_en = re.sub(
                r"w-25,float-start", "image-start", wiki_page.text_en
            )
            wiki_page.save()
