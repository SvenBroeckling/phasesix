...
class MountPoint(HomebrewModel, metaclass=TransMeta):
    ...
    mount_type = models.CharField(_('mount type'), max_length=20, choices=TYPE_CHOICES,
                                  default='weapon')
    ...
    direction = models.CharField(_('direction'), max_length=10,
                                 choices=DIRECTION_CHOICES, default='front')
...
class VehicleMachinery(HomebrewModel, metaclass=TransMeta):
    ...
    required_mount_type = models.CharField(_('required mount type'), max_length=20,
                                           choices=MountPoint.TYPE_CHOICES)
    required_mount_size = models.CharField(_('required mount size'), max_length=10,
                                           choices=MountPoint.SIZE_CHOICES)

    power_consumption = models.IntegerField(_('power consumption'), default=0,
                                            help_text=_('Energy units required'))
    ...
    image = models.ImageField(_('image'), upload_to='vehicle_machinery_images',
                              max_length=256, blank=True, null=True)
    image_copyright = models.CharField(_('image copyright'), max_length=40, blank=True,
                                       null=True)
    image_copyright_url = models.CharField(_('image copyright url'), max_length=150,
...
class Vehicle(HomebrewModel, metaclass=TransMeta):
    ...
    crew_required = models.IntegerField(_('crew required'), default=1, help_text=_(
        'Minimum number of crew needed to operate'))
    passenger_capacity = models.IntegerField(_('passenger capacity'), default=0)
    cargo_capacity = models.FloatField(_('cargo capacity'), default=0,
                                       help_text=_('Cargo capacity in kg'))

    max_speed = models.IntegerField(_('maximum speed'), default=0,
                                    help_text=_('Maximum speed in km/h'))
    acceleration = models.FloatField(_('acceleration'), default=0,
                                     help_text=_('Acceleration rate'))
    handling = models.IntegerField(_('handling'), default=0,
                                   help_text=_('Handling rating'))
    durability = models.IntegerField(_('durability'), default=10,
                                     help_text=_('Vehicle health points'))
    ...
    fuel_efficiency = models.FloatField(_('fuel efficiency'), default=0,
                                        help_text=_('Distance per unit of fuel'))
    range = models.IntegerField(_('range'), default=0,
                                help_text=_('Maximum range in km'))

    energy_capacity = models.IntegerField(_('energy capacity'), default=0,
                                          help_text=_('Energy storage capacity'))
    power_output = models.IntegerField(_('power output'), default=0,
                                       help_text=_('Power generated'))
    ...
    evasion = models.IntegerField(_('evasion'), default=0,
                                  help_text=_('Ability to avoid attacks'))
    stealth = models.IntegerField(_('stealth'), default=0,
                                  help_text=_('Stealth rating'))
    ...
    rarity = models.CharField(_('rarity'), max_length=10, choices=RARITY_CHOICES,
                              default='common')
    price = models.IntegerField(_('price'), default=0)
    ...
    image = models.ImageField(_('image'), upload_to='vehicle_images', max_length=256,
                              blank=True, null=True)
    image_copyright = models.CharField(_('image copyright'), max_length=40, blank=True,
                                       null=True)
    image_copyright_url = models.CharField(_('image copyright url'), max_length=150,
    ...
    tech_level = models.IntegerField(_('tech level'), choices=TECH_LEVEL_CHOICES,
                                     default=2)
    ...
    def get_mounted_weapons(self):
        ...
        return self.vehiclemountpoint_set.filter(
            mounted_machinery__isnull=False,
            mounted_machinery__weapon__isnull=False
        )
    ...
    def get_mounted_equipment(self):
        ...
        return self.vehiclemountpoint_set.filter(
            mounted_machinery__isnull=False,
            mounted_machinery__item__isnull=False
        )
...
class VehicleMountPoint(models.Model):
    ...
    custom_name = models.CharField(_('custom name'), max_length=100, blank=True,
                                   null=True)
    ...
    def is_compatible(self):
        ...
        return (
            self.mounted_machinery.required_mount_type == self.mount_point.mount_type and
            self.mounted_machinery.required_mount_size == self.mount_point.size
        )
...
class VehicleStatusEffect(models.Model, metaclass=TransMeta):
    ...
    duration = models.IntegerField(_('duration'), default=1,
                                   help_text=_('Duration in rounds, -1 for permanent'))
